...
def create_visual (intervals, cln_index, cln_columns, cln_values):
    ...
    for country, interval, query in intervals:
        ...
        plt.title(f'Queue Length in {country.capitalize()} Over Time by Buffer Zone - {interval}', fontsize=30)
...
def create_visual_archive (intervals, cln_index, cln_columns, cln_values):
    
    conn = get_database_connection()

    for country, interval, buffer_zones in intervals:
        query = f"""SELECT * FROM get_car_queue_length_archive('{interval}', {buffer_zones});"""
        ...
        plt.title(
            f'Queue Length in {country.capitalize()} Over Time by Buffer Zone - {datetime.strptime(interval, "%Y-%m-%d").strftime("%B %Y")}',
            fontsize=30
        )
        plt.xticks(rotation=45, fontsize=20)
        plt.yticks(fontsize=20)
        ...
        # Save figure
        plt.savefig(f'/visualizer/www/archive/{country}/queue_length_visual_{interval}.png')
        plt.close()
